...
public class SequentialRemove<E> extends RemoveBase<E>{
    ...
    public Object[] getChanges() {
        return concatenateChanges();
    }

    private Object[] concatenateChanges() {
        if (changes.length == 0) return new Object[0];
        else if (concatenated) return allToRemove;
        ...
        concatenated = true;
        allToRemove = Arrays.copyOf(result, k);
    ...
    public boolean matches(Object o) {
        for (Change<E> change : changes) {
            if (change.matches(o)) return true;
        }
        return false;
    }
